		...
		mContext.sendOrderedBroadcast(new Intent(MainActivity.DATA_REFRESHED_ACTION), null,
				new BroadcastReceiver() {
					...
					public void onReceive(Context context, Intent intent) {
						...
						// DO: Check whether or not the MainActivity
						// received the broadcast

						if (getResultCode() != MainActivity.IS_ALIVE) {

							// DO: If not, create a PendingIntent using
							// the
							// restartMainActivityIntent and set its flags
							// to FLAG_UPDATE_CURRENT

							PendingIntent penintent = PendingIntent.getActivity(mContext, 0, restartMainActivityIntent, PendingIntent.FLAG_UPDATE_CURRENT);
							...
							// DO: Set the notification View's text to
							// reflect whether the download completed
							// successfully

							mContentView.setTextViewText(R.id.text42, "Download Complete FUCK!!!");


							// DO: Use the Notification.Builder class to
							...
							NotificationManager mNotifMan = (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);

							mNotifMan.notify(MY_NOTIFICATION_ID, notificationBuilder.build());
